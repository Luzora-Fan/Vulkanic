...
private:
    ...
    void CreateShaderBindingTables()
    {
        ...
        auto createSingleRecordRegion = [&](uint32_t groupIndex,
                                            BufferAllocation& buffer,
        ...
        createSingleRecordRegion(0, m_raygenShaderBindingTable, m_raygenShaderBindingTableRegion);

        std::vector<uint8_t> missRecords(static_cast<size_t>(recordSize) * 2u, 0);
        for (uint32_t missGroupIndex = 0; missGroupIndex < 2; ++missGroupIndex)
        {
            std::memcpy(missRecords.data() + static_cast<size_t>(recordSize) * missGroupIndex,
                        shaderGroupHandles.data() + static_cast<size_t>(1u + missGroupIndex) * handleSize,
                        handleSize);
        }
        ...
        createSingleRecordRegion(3, m_hitShaderBindingTable, m_hitShaderBindingTableRegion);
